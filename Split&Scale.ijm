 ...
 * 
 * ABOUT
 * This macro takes a folder of stacks and extracts a channel, runs CLAHE, adds a scale bar and saves. 
 * Stacks are saved to an output folder with a new name extension.
 * 
 * Originally used for figure prep for ROIs 
 * 
 */
...
process_as_composite = true;
	...
	Dialog.addNumber("Size of px in um:",0.06); 
			...
			for ( f=1; f<=frames; f++ ) {
			  ...
			  for ( s=1; s<=slices; s++ ) {
			    ...
			    for ( c=1; c<=channels; c++ ) {
			      ...
			      run("Set Scale...", "distance=1 known=&px_size unit=um global");
			      run("Scale Bar...", "width=2 height=5 thickness=10 font=14 color=Black background=None location=[Lower Right] horizontal bold hide overlay");
